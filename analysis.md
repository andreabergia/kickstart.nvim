...
- [x] **`autocmd-restore-position.lua` and `autocmd-highlight-post-copy.lua`** — checked on Neovim 0.12.2; no built-in `TextYankPost` highlight autocmd is active under `--clean`, and restore-position is custom. Keep both.
